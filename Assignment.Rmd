---
title: "Practical Machine Learning Assignment"
author: "Ken H."
date: "2015-09-23"
output: 
  html_document: 
    keep_md: yes
---

```{r libraries, warning=FALSE, echo=FALSE, message=FALSE}
# load libraries needed for analysis
library(doParallel)
library(caret)
library(knitr)
library(readr)

# set default chunk options
opts_chunk$set(warning=FALSE, echo=FALSE, message=FALSE, eval=TRUE, cache=TRUE)
```

```{r get_data}
if (!file.exists("pml-training.csv")) {
  download.file("https://d396qusza40orc.cloudfront.net/predmachlearn/pml-training.csv",
                "pml-training.csv")
  download.file("https://d396qusza40orc.cloudfront.net/predmachlearn/pml-testing.csv",
                "pml-testing.csv")
}
pml.training <- read_csv("./pml-training.csv")
pml.testing <- read_csv("./pml-testing.csv")
```

```{r clean_data}
sumcols <- grep('^(var|avg|max|min|kurtosis|skewness|amplitude|stddev)', colnames(pml.training))
pml.training <- pml.training[, -c(1:7, sumcols)]
pml.training$classe <- as.factor(pml.training$classe)
```

```{r split_data}
set.seed(3777)
inTrain <- createDataPartition(y = pml.training$classe, p = 0.80, list = FALSE)
training <- pml.training[inTrain, ]
testing <- pml.training[-inTrain, ]
```

```{r train_model}
seeds <- vector(mode = "list", length = 11)
for (i in 1:10) seeds[[i]] <- sample.int(n = 1000, 51)

seeds[[11]] <- sample.int(1000, 1)

myControl <- trainControl(method = 'oob', seeds = seeds, index = createFolds(training$classe))

cl <- makeCluster(detectCores())
registerDoParallel(cl)
# repeated the following with 'gbm', 'knn', and 'lda' methods, but 'rf' gives the best accuracy
model <- train(classe ~ ., training, method = 'rf', trControl = myControl, tuneLength = 10)
stopCluster(cl)
model
```

```{r test_model}
prediction <- predict(model, testing)
confusionMatrix(prediction, testing$classe)
```

```{r answer_questions, echo=FALSE}
# Predict the answers to the assignment
assignment <- as.character(predict(model, pml.testing))

# Output the assignment answers to individually named files
n <- length(assignment)
for (i in 1:n) {
        write.table(assignment[i], 
                    file = sprintf("problem_id_%i.txt", i), 
                    row.names = FALSE, col.names = FALSE, quote = FALSE)
}

```

